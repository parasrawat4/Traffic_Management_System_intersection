...
def count():
    ...
    net = cv2.dnn.readNet("D:\V Semester\ML project\Traffic Management\mod\FinalModel\yolov3.weights", "D:\V Semester\ML project\Traffic Management\mod\FinalModel\yolov3.cfg") #specify the path for weight and cfg file
    classes = []                                                                   #for stroing various things yolo can detect

    with open("D:\V Semester\ML project\Traffic Management\mod\FinalModel\coco.names", "r") as namef: #extracting name of various oject yolo can detect from its coco.name file
    ...
    cv2.imwrite('C:/Users/Paras Rawat/OneDrive/Desktop/clone'+str(imnum)+'.png', img) #storing this image

    img = cv2.imread('C:/Users/Paras Rawat/OneDrive/Desktop/clone'+str(imnum)+'.png') #using this image
    ...
    for out in outs:
        for detection in out:              # finding detections from the scores
            scores = detection[5:]         #seeing the score for detection for all classes that yolo can detect
    ...
    for i in range(len(boxes)):                             #counting number of cars/trucks/bicycle etc in our image
        if i in indexes:
            ...
            if(label=="car"):                               #if car car count increase
                car+=1
                vehicle+=1
                print("No of car detected",car)
            if(label=="truck"):                             #if truck truck count increase
                truck+=1
                vehicle+=1
                print("No of truck detected",truck)
            if(label=="bicycle"):                           #if bicycle bicycle count increase
                bicycle+=1
                vehicle+=1
                print("No of bicycle detected",bicycle)
            if(label=="bus"):                               #if bus  count increase
                bus+=1
                vehicle+=1
                print("No of bus detected",bus)
            if(label=="motorbike"):                         #if motorbike motorbike count increase
                bus+=1
                vehicle+=1
                print("No of motorbike detected",motorbike)
